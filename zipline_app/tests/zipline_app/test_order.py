...
class OrderModelTests(TestCase):
    ...
    def test_order_with_user(self):
      ...
      o1 = create_order(days=-1, asset=self.a1a, order_side=BUY, amount_unsigned=1, order_text="order 1", user=user, account=self.acc1)
...
class OrderGeneralViewsTests(TestCase):
    ...
    def test_new_post(self):
        ...
        url = reverse('zipline_app:orders-new')
        response = self.client.post(url, {'pub_date':time, 'asset':self.a1a.id, 'order_side': BUY, 'amount_unsigned':10, 'account':self.acc1.id, 'order_type': MARKET})
...
#        self.assertFormError(response, 'form', 'pub_date', 'Enter a valid date/time.')
    ...
    def test_quantity_large_does_not_trigger_error_integer_too_large(self):
        ...
        response = self.client.post(url, {'pub_date':time, 'asset':self.a1a.id, 'order_side': BUY, 'amount_unsigned':largeqty, 'account':self.acc1.id})
    ...
    def test_new_order_timezone(self):
        ...
        o1={'pub_date':time, 'asset':self.a1a.id, 'order_side': BUY, 'amount_unsigned':1, 'account':self.acc1.id}
    ...
    def test_new_order_zeroqty(self):
        url = reverse('zipline_app:orders-new')
        time = '2015-01-01 06:00:00'
        o1={'pub_date':time, 'asset':self.a1a.id, 'order_side': BUY, 'amount_unsigned':0, 'account':self.acc1.id}
    ...
    def test_new_order_user(self):
        url = reverse('zipline_app:orders-new')
        time = '2015-01-01 06:00:00'
        o1={'pub_date':time, 'asset':self.a1a.id, 'order_side': BUY, 'amount_unsigned':1, 'account':self.acc1.id, 'order_type': MARKET}
...
class OrderDetailViewTests(TestCase):
    ...
    def test_detail_view_with_a_future_order(self):
        ...
        """
        future_order = create_order(order_text='Future order.', days=5, asset=self.a1a, order_side=BUY, amount_unsigned=10, account=self.acc1)
    ...
    def test_detail_view_with_a_past_order(self):
        ...
        """
        past_order = create_order(order_text='Past Order.', days=-5, asset=self.a1a, order_side=BUY, amount_unsigned=10, account=self.acc1)
    ...
    def test_detail_view_history(self):
      order = create_order(order_text='original order.', days=-5, asset=self.a1a, order_side=BUY, amount_unsigned=10, account=self.acc1)
