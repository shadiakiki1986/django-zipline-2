...
class ZlModelMethodTests(TestCase):
    ...
    def test_update_some_orders_some_fills(self):
        o1 = create_order(order_text="test?",days=-0.5, asset=self.a1a, order_side=BUY, order_qty_unsigned=10, account=self.acc1)
        o2 = create_order(order_text="test?",days=-0.5, asset=self.a2a, order_side=BUY, order_qty_unsigned=10, account=self.acc1)
        f1 = create_fill(fill_text="test?",days=-0.5, asset=self.a1a, fill_side=BUY, fill_qty_unsigned=2, fill_price=2)
        f2 = create_fill(fill_text="test?",days=-0.5, asset=self.a2a, fill_side=BUY, fill_qty_unsigned=2, fill_price=2)

        self.assertEqual(len(ZlModel.zl_open), 2)
        self.assertEqual(len(ZlModel.zl_closed), 0)

    def test_update_fills_then_orders(self):
        f1 = create_fill(fill_text="test?",days=-0.5, asset=self.a1a, fill_side=BUY, fill_qty_unsigned=20, fill_price=2)
        f2 = create_fill(fill_text="test?",days=-0.5, asset=self.a2a, fill_side=BUY, fill_qty_unsigned=20, fill_price=2)
        o1 = create_order(order_text="test?",days=-0.5, asset=self.a1a, order_side=BUY, order_qty_unsigned=10, account=self.acc1)
        o2 = create_order(order_text="test?",days=-0.5, asset=self.a2a, order_side=BUY, order_qty_unsigned=10, account=self.acc1)
    ...
    def test_order_deleted(self):
        o1a = create_order(order_text="test?",days=-0.5, asset=self.a1a, order_side=BUY, order_qty_unsigned=10, account=self.acc1)
        ...
        o1b = create_order(order_text="test?",days=-0.5, asset=self.a1a, order_side=BUY, order_qty_unsigned=10, account=self.acc1)
        self.assertEqual(len(ZlModel.orders), 1)
        self.assertEqual(self.a1a.id in ZlModel.orders, True)
